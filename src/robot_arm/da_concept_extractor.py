...
from robot_arm.crf_util import word2features, sent2features, sent2labels
import re
...
class DA_Concept:
    ...
    def __init__(self):
        ...
        # SVMモデルの読み込み
        with open("/home/takuma/catkin_ws/src/robot_arm/scripts/svc/svc.model","rb") as f:
            self.vectorizer = dill.load(f)
            self.label_encoder = dill.load(f)
            self.svc = dill.load(f)

        # CRFモデルの読み込み
        with open("/home/takuma/catkin_ws/src/robot_arm/scripts/crf.model","rb") as f:
